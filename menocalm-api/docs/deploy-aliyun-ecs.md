# 在阿里云部署 MenoCalm API（`menocalm-api`，ECS + 托管数据）
...
| **容器镜像服务 ACR** | 存放 `menocalm-api` 镜像（个人版实例即可起步） |
...
在开发机（已安装 Docker）的 **`menocalm-api` 目录**执行：

```bash
docker build -t menocalm-api:0.1.0 .
```
...
docker tag menocalm-api:0.1.0 registry.cn-hangzhou.aliyuncs.com/你的命名空间/menocalm-api:0.1.0
docker push registry.cn-hangzhou.aliyuncs.com/你的命名空间/menocalm-api:0.1.0
```
...
```bash
docker pull registry.cn-hangzhou.aliyuncs.com/你的命名空间/menocalm-api:0.1.0
docker run -d --name menocalm-api --restart unless-stopped -p 8080:8080 \
  ...
  registry.cn-hangzhou.aliyuncs.com/你的命名空间/menocalm-api:0.1.0
```
