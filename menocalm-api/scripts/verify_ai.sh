...
rm -f /tmp/menocalm-api.pid /tmp/menocalm-api.log /tmp/an.json

mvn -q -DskipTests package

(mvn -q spring-boot:run > /tmp/menocalm-api.log 2>&1 & echo $! > /tmp/menocalm-api.pid)
PID="$(cat /tmp/menocalm-api.pid)"
...
kill "$PID"
sleep 0.5
tail -n 30 /tmp/menocalm-api.log || true
