...
public class ConversationService {
    ...
    private final MenopauseAnalyticsWriter analyticsWriter;
    private final ZoneId dailyContextZone;

    public ConversationService(
            ConversationStore conversationStore,
            ConversationAnalysisClient analysisClient,
            FallbackConversationInsightGenerator fallbackGenerator,
            MenopauseAnalyticsWriter analyticsWriter,
    ...
    public ConversationMessageDto appendMessage(
        ...
        if ("user".equalsIgnoreCase(role)) {
            analyticsWriter.recordAfterUserMessage(queryUserId, message);
        }
        return message;
    }
    ...
    public ConversationInsightDto analyzeAndUpsertInsight(
        ...
        assertSameUser(authenticatedUserId, queryUserId);
        String normalizedConversationId = requireConversationId(conversationId);
        List<ConversationMessageDto> allMessages = conversationStore.listMessages(queryUserId, normalizedConversationId, 200);
        if (allMessages.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "conversation_empty");
        }
        List<ConversationMessageDto> messages = ConversationDailyContext.sameCalendarDayAsLatest(allMessages, dailyContextZone);
        if (messages.isEmpty()) {
        ...
        try {
            ...
            conversationStore.upsertInsight(queryUserId, normalizedConversationId, insight);
            analyticsWriter.recordInsightSnapshot(queryUserId, normalizedConversationId, insight);
            return insight;
        } catch (ResponseStatusException ex) {
            if (ex.getStatusCode().value() == 503) {
                // Always regenerate from current messages. Returning a cached insight would
                // answer a previous turn after transient AI failures — very poor chat quality.
                ...
                analyticsWriter.recordInsightSnapshot(queryUserId, normalizedConversationId, fallback);
                return fallback;
            }
            throw ex;
        }
