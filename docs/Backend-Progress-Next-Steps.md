...
- `live-more-api/` 才是当前应继续开发的主后端工程。
- `backend/backend` 是后续临时初始化的最小脚手架，不应作为主线。
- 建议后续所有后端开发、联调、CI 全部收敛到 `live-more-api/`。

---

## 2. 当前真实进度（基于 live-more-api）
...
## P0（立即）
- 明确“单后端目录”决策：主后端固定为 `live-more-api/`
...
## P2（1-2 周）
- 将 `live-more-api` 纳入仓库 CI（build + test + basic security gate）
...
- iOS 端运行时 `backend baseURL` 指向 `live-more-api` 服务地址
- 先验证四条主链路：
  - 手机号登录
  - token 刷新
  - 获取日志列表
...
- 在仓库根目录新增一条规则：后端主目录为 `live-more-api/`
- 给 `live-more-api/README.md` 增补“启动 + 环境变量 + API 列表 + 联调步骤”
